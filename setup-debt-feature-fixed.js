...
`;
        ...
        // Insertar antes del middleware de errores 404
...
function updateAnalysisTemplate() {
    ...
    if (!fs.existsSync(analysisPath)) {
        console.log('⚠️ Archivo analisis.ejs no encontrado, omitiendo...');
        return;
    }
    ...
    if (!analysisContent.includes('debt-capacity-button')) {
        ...
        if (analysisContent.includes('Descargar PDF')) {
...
function updatePlansPage() {
    ...
    if (!fs.existsSync(plansPath)) {
        console.log('⚠️ Archivo planes.ejs no encontrado, omitiendo...');
        return;
    }
    ...
    if (!plansContent.includes('Capacidad de endeudamiento')) {
        ...
        if (plansContent.includes('Templates por industria')) {
...
try {
    ...
    console.log('2. Ir a /planes y suscribirse a premium');
    console.log('3. Probar funcionalidad en análisis');
    console.log('\n🚀 ¡Tu sistema está listo para dominar el mercado!');
