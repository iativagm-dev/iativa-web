...
const insertAdmin = db.prepare(`INSERT OR IGNORE INTO users (username, email, password, full_name, is_admin) 
        VALUES (?, ?, ?, ?, ?)`);
insertAdmin.run('admin', 'admin@iativa.com', adminPassword, 'Administrador IAtiva', 1);
...
const insertAnalytics = db.prepare(`INSERT INTO analytics (event_type, user_id, session_id, data, ip_address, user_agent, created_at) 
...
function logAnalytics(eventType, req, data = {}) {
    ...
    insertAnalytics.run(analytics.event_type, analytics.user_id, analytics.session_id, 
